...
// Smooth scroll para el botón del mapa
document.querySelector('#ver-mapa').addEventListener('click', (e) => {
  e.preventDefault();
...
// Cargar dinámicamente los enlaces de materiales
const enlacesAnios = document.querySelectorAll('.enlace-anio');
const cursos = {
  'Primer año': 'https://ejemplo.com/primero',
  'Segundo año': 'https://ejemplo.com/segundo',
  'Tercer año': 'https://ejemplo.com/tercero',
  'Cuarto año': 'https://ejemplo.com/cuarto'
};

enlacesAnios.forEach(enlace => {
  const texto = enlace.textContent;
  enlace.href = cursos[texto];
  enlace.target = '_blank';
});

// Acordeón para pasos de matrícula
document.querySelectorAll('.paso-card').forEach((paso, index) => {
  paso.addEventListener('click', () => {
    const contenido = `<div class="paso-detalle">
      <p>Contenido detallado del paso ${index + 1}...</p>
    </div>`;
    
    if(paso.querySelector('.paso-detalle')) {
      paso.querySelector('.paso-detalle').remove();
    } else {
      paso.insertAdjacentHTML('beforeend', contenido);
    }
  });
});
});